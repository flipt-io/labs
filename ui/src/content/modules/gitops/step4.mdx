...
Now that we have successfully enabled our chatbot, we can review how to configure Flipts other resources necessary for the `chat-personas` feature flag.

Checkout the second pull-requests content at: [http://localhost:3001/flipt/features/pulls/2](http://localhost:3001/flipt/features/pulls/2/files).

Here we are defining the `chat-personas` feature flag with the same state we arrived at in the advanced tutorial.
