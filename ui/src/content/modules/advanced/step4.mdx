...
This is because we are passing the username to our backend API which is then including it as the `entityId` in it's request to the Flipt evaluation endpoint.
...
Above, we make a request to Flipt using [flipt-python](https://github.com/flipt-io/flipt-python) to evaluate the `chat-personas` flag with the `entityId` we've been passed.
...
Flipt evaluations are also "sticky" based on the user in this example, which is why you'll see users maintaining the sentiment they were assigned at first!
